...
// ══ DEBUG — DEVE SER A PRIMEIRA COISA A RODAR ══
window.__dbgLines = [];
function dbg(msg, color){
  color = color||'#7cfc6e';
  var t = new Date().toLocaleTimeString('pt-BR');
  var txt = '['+t+'] '+String(msg);
  window.__dbgLines.push({txt:txt,color:color});
  var el = document.getElementById('debugLog');
  if(el){ var d=document.createElement('div');d.style.color=color;d.style.marginBottom='2px';d.textContent=txt;el.appendChild(d);el.scrollTop=el.scrollHeight; }
  console.log(txt);
}
window.onerror = function(msg,src,line,col,err){
  dbg('❌ ERROR: '+msg+' | '+src+':'+line, '#ff6b6b');
  var p=document.getElementById('debugPanel'); if(p) p.style.display='block';
  return false;
};
window.addEventListener('unhandledrejection',function(e){
  dbg('❌ PROMISE: '+(e.reason?e.reason.toString():String(e)), '#ff9f43');
  var p=document.getElementById('debugPanel'); if(p) p.style.display='block';
});
dbg('✅ Script top reached');
...
function renderDashboard(){
  ...
  document.getElementById('metricsArea').innerHTML=`
    ...
    <div class="metric m-blue"><div class="metric-label">Gasto médio</div><div class="metric-val">${R(avg)}</div><div class="metric-sub">por transação</div></div>
    ...
    <div class="metric m-green"><div class="metric-label">Cartões ativos</div><div class="metric-val">${cards.length}</div><div class="metric-sub">cadastrados</div></div>`;
...
function init(){
  ...
  dbg('btnHamburger found: '+(btnH?'YES':'NO'));
  ...
  // ── Load data ──
  dbg('loadLocal + render starting');
  loadLocal();
  render();
  dbg('render done, calling loadFromDB');
...
dbg('✅ DOMContentLoaded listener registered');
document.addEventListener('DOMContentLoaded', function(){ dbg('✅ DOM ready, calling init'); init(); });
