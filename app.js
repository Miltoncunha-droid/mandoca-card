...
document.getElementById(‘metricsArea’).innerHTML=` <div class="metric m-orange"><div class="metric-label">Total do mês</div><div class="metric-val">${R(total)}</div><div class="metric-sub">${count} lançamento${count!==1?'s':''}</div></div> <div class="metric m-blue"><div class="metric-label">Gasto médio</div><div class="metric-val">${R(avg)}</div><div class="metric-sub">por transação</div></div> <div class="metric m-purple"><div class="metric-label">Débito futuro total</div><div class="metric-val">${R(futDebt)}</div><div class="metric-sub">parcelas restantes</div></div> <div class="metric m-green"><div class="metric-label">Cartões ativos</div><div class="metric-val">${cards.length}</div><div class="metric-sub">cadastrados</div></div>`;
...
el.innerHTML=` <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;"> <span style="font-size:1.4rem;line-height:1">${p.emoji}</span> <div class="pc-name c${ci}" style="margin:0">${p.name}</div> </div> <div class="pc-month">${R(pT)}</div> <div class="pc-total">no mês · ${pPct.toFixed(1)}%</div> ${pDbt>0?`<div class="pc-debt c${ci}">Débito futuro: ${R(pDbt)}</div>`:''} <div class="bar-bg"><div class="bar-fill bg${ci}" style="width:${pPct}%"></div></div> <button class="btn-print" onclick="event.stopPropagation();printExtrato('${p.name.replace(/'/g,"\\'")}')">🖨️ Imprimir Extrato</button>`;
grid.appendChild(el);
});
...
row.innerHTML=` <div class="exp-dot bg${ci}"></div> <div class="exp-info"><div class="name">${exp.desc}</div><div class="meta">${exp.cat} · ${date}</div></div> <div class="exp-card-tag">💳 ${cardName}</div> <div class="exp-person-tag c${ci}">${personEmoji(exp.person)} ${exp.person}</div> <div class="exp-inst">${inst}</div> <div class="exp-amount c${ci}">${R(showParcel?exp._amount:exp.amount)}</div> <div style="display:flex;align-items:center;gap:8px;border-left:1px solid var(--border);padding-left:12px;margin-left:4px;"> <button class="btn-icon edit" onclick="editExpense('${exp.id}')" title="Editar gasto">✎</button> <button class="btn-icon btn-del" onclick="confirmDeleteExpense('${exp.id}','${exp.desc.replace(/'/g,"\\'")}')" title="Excluir gasto">✕</button> </div>`;
return row;
}
...
document.getElementById(‘debtMetrics’).innerHTML=` <div class="metric m-purple"><div class="metric-label">Débito restante</div><div class="metric-val">${R(tRem)}</div><div class="metric-sub">este mês + futuro</div></div> <div class="metric m-green"><div class="metric-label">Já pago</div><div class="metric-val">${R(tPaid)}</div><div class="metric-sub">faturas anteriores</div></div> <div class="metric m-orange"><div class="metric-label">Total de compras</div><div class="metric-val">${R(tAll)}</div><div class="metric-sub">soma bruta</div></div> <div class="metric m-blue"><div class="metric-label">Parcelas em aberto</div><div class="metric-val">${all.filter(r=>r._billYear*12+r._billMonth>=nk).length}</div><div class="metric-sub">lançamentos</div></div>`;
...
ms.innerHTML=`<div style="font-size:.67rem;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:${isView?'var(--a1)':'var(--muted)'};margin-bottom:8px;display:flex;align-items:center;gap:8px;"> ${mLabel.toUpperCase()} ${isView?'<span style="color:var(--a1);background:var(--a1l);border:1px solid rgba(212,99,10,.3);border-radius:6px;padding:1px 8px;font-size:.55rem;">MÊS ATUAL</span>':''} <span style="margin-left:auto;color:var(--a1);font-weight:700">${R(mT)}</span></div> ${g.rows.map(r=>`<div class="debt-row">
...
el.innerHTML=`<div class="cc-card-shine"></div> <div class="cc-actions"> <button class="btn-icon edit" onclick="editCard('${c.id}')" title="Editar">✎</button> <button class="btn-icon" onclick="deleteCard('${c.id}')" title="Excluir">✕</button> </div> <div><div class="cc-card-brand">${c.name}</div><div class="cc-card-info">${c.brand}</div></div> <div class="cc-card-dates"> <div class="cc-date-block"><div class="lbl">FECHAMENTO</div><div class="val">dia ${c.closeDay}</div></div> <div class="cc-date-block"><div class="lbl">VENCIMENTO</div><div class="val">dia ${c.dueDay}</div></div> </div>`;
grid.appendChild(el);
});
}
...
el.innerHTML=`<div class="prc-actions"> <button class="btn-icon edit" onclick="editPerson('${p.id}')" title="Editar">✎</button> <button class="btn-icon" onclick="deletePerson('${p.id}')" title="Excluir">✕</button> </div> <div class="prc-avatar">${p.emoji}</div> <div class="prc-name c${ci}">${p.name}</div> <div class="prc-meta">Cor: <span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${COLORS[ci]};vertical-align:middle"></span></div>`;
grid.appendChild(el);
});
}
...
const cardBlocks = Object.values(byCard).map(grp=>`<div style="margin-bottom:20px;"> <div style="font-size:11px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase; color:#666;border-bottom:1px solid #e0ddd6;padding-bottom:6px;margin-bottom:10px;"> 💳 ${grp.name} </div> ${grp.items.map(e=>{ const date = e.date ? new Date(e.date+'T12:00:00').toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'}) : ''; const inst = e._isInst ?` <span style="color:#999;font-size:10px">${e._parcel}/${e.installments}x</span>`: ''; return`<div style="display:flex;justify-content:space-between;align-items:baseline;
...
</div>`; }).join('')} <div style="display:flex;justify-content:space-between;padding:8px 0 0;font-size:12px;font-weight:700;color:#666;"> <span>Subtotal ${grp.name}</span> <span>${R(grp.total)}</span> </div> </div> `).join(’’);
